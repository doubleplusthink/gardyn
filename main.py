...
from config import NETWORK_PATH, CONN_STR_FILE, LIGHT_SCHEDULE_FILE, PUMP_SCHEDULE_FILE, LIGHT_STATUS, PUMP_STATUS
...
def thread_pump(pump_q, status_q):
...
def thread_button_controller(light_q):
    ...
    def button_handler():
        ...
        led_button_status[0] = 'OFF' if (led_status or led_button_status[0] == 'ON') else 'ON'
...
async def main():
    ...
    worker_pump = Thread(target=thread_pump, args=(pump_q, status_q))
    ...
    if conn_string:
        ...
        listener = IoTDMListener(iot_client, light_q, pump_q, light_event, pump_event)
        listener.run()
